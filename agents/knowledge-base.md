...
This file contains domain knowledge about the Kalo product and codebase. For coding guidelines and rules, see [`rules/`](rules/).
...
Kalo events in Google Calendar can be identified by checking if the iCalUID ends with `@Kalo` (e.g., `2GBXSdEixretciJfKVmYN8@Kalo`). This identifier is used to distinguish Kalo bookings from other calendar events for data storage and privacy purposes.
...
Workflows and webhooks are two completely separate features in Kalo with different implementations and file structures:
