...
import { CREDENTIAL_SYNC_SECRET, CREDENTIAL_SYNC_SECRET_HEADER_NAME } from "@kalo/lib/constants";
...
const appCredentialWebhookRequestBodySchema = z.object({
  ...
  userId: z.number().int(),
  appSlug: z.string(),
  // Keys should be AES256 encrypted with the KALO_APP_CREDENTIAL_ENCRYPTION_KEY
  keys: z.string(),
});
...
async function postHandler(request: NextRequest) {
  ...
  try {
    ...
    const keys = JSON.parse(
      symmetricDecrypt(reqBody.keys, process.env.KALO_APP_CREDENTIAL_ENCRYPTION_KEY || "")
    );
