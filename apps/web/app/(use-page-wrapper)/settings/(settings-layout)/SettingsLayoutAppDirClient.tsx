...
import { HAS_ORG_OPT_IN_FEATURES } from "@kalo/features/feature-opt-in/config";
import type { TeamFeatures } from "@kalo/features/flags/config";
...
import { VerticalTabItem } from "@kalo/ui/components/navigation";
  ...
  const tabs: VerticalTabItemProps[] = [
    ...
    },
    {
      ...
      children: [
        {
          name: "alerts",
          href: "/settings/vendor/alerts",
          trackingMetadata: { section: "vendor", page: "alerts" },
        },
        {
          name: "inventory",
          href: "/settings/vendor/inventory",
          trackingMetadata: { section: "vendor", page: "inventory" },
        },
        {
          name: "receipts",
          href: "/settings/vendor/receipts",
          trackingMetadata: { section: "vendor", page: "receipts" },
        },
      ],
    },
    {
...
const useTabs = ({
  ...
  const { data: unreadAlerts } = trpc.viewer.vendor.getUnreadAlertsCount.useQuery();
  ...
  const processTabsMemod = useMemo(() => {
    const processedTabs = getTabs(orgBranding).map((tab) => {
      if (tab.href === "/settings/my-account") {
        ...
        return { ...tab, children: filtered };
      } else if (tab.href === "/settings/vendor") {
        return {
          ...tab,
          children: (tab?.children ?? []).map((child) =>
            child.href === "/settings/vendor/alerts" ? { ...child, count: unreadAlerts?.count ?? 0 } : child
          ),
        };
      }
      return tab;
  ...
  }, [isAdmin, orgBranding, user, unreadAlerts, isDelegationCredentialEnabled, isPbacEnabled, permissions]);
                ...
                <div key={child.href} className="flex items-start gap-2">
                  ...
                  {child.count !== undefined && child.count > 0 && (
                    <Badge
                      ...
                      data-testid={`unread-badge-${child.href}`}>
                      {child.count}
                    </Badge>
                  )}
                </div>
