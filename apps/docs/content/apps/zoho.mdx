...
```
<Kalo URL>/api/integrations/zohocrm/callback
```

Replace `<Kalo URL>` with your application URL.
...
10. **Integration is Ready** - Your ZohoCRM integration can now be easily added in the Kalo settings.
...
```
<Kalo URL>/api/integrations/zohocalendar/callback
```
...
5. **Save Credentials in Kalo Admin Panel** - Copy the Client ID and Client Secret to your app keys in the Kalo admin panel at:

```
<Kalo>/settings/admin/apps
```
...
```
<Kalo URL>/api/integrations/zoho-bigin/callback
```
...
7. **Complete Zoho Bigin Integration** - Your Zoho Bigin integration is now ready and can be added from the Kalo app store.
