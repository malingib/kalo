...
```
<Kalo URL>/api/integrations/googlecalendar/callback
<Kalo URL>/api/auth/callback/google
```

Replace `<Kalo URL>` with the URL where your application runs.
...
### Adding Google Calendar to Kalo App Store
