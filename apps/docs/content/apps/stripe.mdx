...
4. **Add the redirect URL** - Add the following redirect URL, replacing `<Kalo URL>` with your application's URL:

```
<Kalo URL>/api/integrations/stripepayment/callback
```
...
```
<Kalo URL>/api/integrations/stripepayment/webhook
```
