...
- [x] Unlocked attribute shouldn't get updates from SCIM but in case they still do, we follow this strategy
    ...
    - [x] The options created by SCIM aren't deletable from Kalo UI. User can only remove options created by Kalo i.e. where AttributeToUser.createdByDSyncId is null.
...
## Documentation
Okta stuff
- Removing the assignment of a user from the app, doesn't remove the user from Kalo
