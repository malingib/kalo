...
import { ZGetPlatformSettingsSchema, ZUpdatePlatformSettingsSchema } from "./platformSettings.schema";
...
export const adminRouter = router({
  ...
  getPlatformSettings: authedAdminProcedure.input(ZGetPlatformSettingsSchema).query(async (opts) => {
    const { getPlatformSettingsHandler } = await import("./platformSettings.handler");
    return getPlatformSettingsHandler(opts);
  }),
  updatePlatformSettings: authedAdminProcedure.input(ZUpdatePlatformSettingsSchema).mutation(async (opts) => {
    const { updatePlatformSettingsHandler } = await import("./platformSettings.handler");
    return updatePlatformSettingsHandler(opts);
  }),
});
