...
import { buildBookingContext } from "../handleNewBooking/buildBookingContext";
...
async function handler(
  ...
  const bookingContext = buildBookingContext({
    ...
    reqBody: {
      ...
      skipContactOwner: reqBody.skipContactOwner,
  ...
  try {
    await checkIfBookerEmailIsBlocked({
      ...
      bookerEmail: bookingContext.bookerEmail,
  ...
  spamCheckService.startCheck({
    ...
    organizationId: bookingContext.eventTypeOrganizationId,
  });
  ...
  if (eventType.requiresBookerEmailVerification && !rawBookingData.rescheduleUid) {
    ...
    try {
      await verifyCodeUnAuthenticated(bookingContext.bookerEmail, verificationCode);
    } catch {
      throw new HttpError({
        statusCode: 400,
  ...
  const fullName = bookingContext.fullName;
  // Why are we only using "en" locale
  const tGuests = await getTranslation("en", "common");

  const dynamicUserList = bookingContext.dynamicUserList;
  ...
  if (
    ...
    const existingBooking = await deps.bookingRepository.getValidBookingFromEventTypeForAttendee({
      eventTypeId,
      bookerEmail: bookingContext.bookerEmail,
      bookerPhoneNumber: bookingContext.bookerPhoneNumber,
  ...
  const contactOwnerEmail = bookingContext.contactOwnerEmail;
  ...
  const { qualifiedRRUsers, additionalFallbackRRUsers, fixedUsers } = await loadAndValidateUsers({
    ...
    contactOwnerEmail: bookingContext.contactOwnerEmail,
  ...
  const attendeeInfoOnReschedule =
    userReschedulingIsOwner && originalRescheduledBooking
      ? originalRescheduledBooking.attendees.find((attendee) => attendee.email === bookingContext.bookerEmail)
      : null;
  ...
  const invitee: Invitee = [
    {
      ...
      phoneNumber: bookingContext.bookerPhoneNumber,
  ...
  if (eventType.seatsPerTimeSlot) {
    const newBooking = await handleSeats({
      ...
      organizerUser,
      originalRescheduledBooking,
      bookingContext.bookerEmail,
      bookingContext.bookerPhoneNumber,
      tAttendees,
      bookingSeat,
    ...
    if (!noEmail && isConfirmedByDefault && !isDryRun) {
      await emailsAndSmsHandler.send({
        ...
        data: {
          ...
          originalRescheduledBooking,
          rescheduleReason,
          isRescheduledByBooker: reqBody.rescheduledBy === bookingContext.bookerEmail,
          users,
          changedOrganizer,
        },
      });
  ...
  await this.fireBookingEvents({
    ...
    isDryRun,
    bookerEmail: bookingContext.bookerEmail,
    bookerName: bookingContext.fullName,
    originalRescheduledBooking,
