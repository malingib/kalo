...
import { createWatchlistEntry } from "@kalo/features/watchlist/lib/testUtils";
import { WEBAPP_URL, WEBSITE_URL } from "@kalo/lib/constants";
import { ErrorCode } from "@kalo/lib/errorCodes";
import type { HttpError } from "@kalo/lib/http-error";
import logger from "@kalo/lib/logger";
import { resetTestEmails } from "@kalo/lib/testEmails";
import { distributedTracing } from "@kalo/lib/tracing/factory";
import { BookingStatus, CreationSource, SchedulingType, WatchlistType } from "@kalo/prisma/enums";
import {
...
import { testWithAndWithoutOrg } from "@kalo/testing/lib/bookingScenario/test";
...
async function mockPaymentSuccessWebhookFromStripe({ externalId }: { externalId: string }) {
  const { handlePaymentSuccess } = await import("@kalo/app-store/_utils/payments/handlePaymentSuccess");
...
describe("handleNewBooking", () => {
  ...
  describe("Fresh/New Booking:", () => {
    ...
    describe(
      ...
      () => {
        test(
          `should fail a booking if there is already a Kalo booking overlapping the time`,
          async () => {
            const handleNewBooking = getNewBookingHandler();
